...
def send_submission_notification(
    ...
    """Notify admin of a new submission. Non-blocking — never raises."""
    if not settings.ADMIN_EMAIL or not settings.SMTP_USER:
        logger.info("Email not configured — skipping submission #%s notification", submission_id)
        return False
...
<html><body style="font-family:monospace;background:#03040a;color:#e8e4dc;padding:2rem;">
  ...
  <p style="font-family:Georgia,serif;font-size:1.05rem;line-height:1.8;color:#e8e4dc;white-space:pre-wrap;">{content}</p>
</body></html>
""".strip()
...
def send_signup_notification(username: str, user_email: str, timestamp: datetime) -> bool:
    """Notify admin when a new user signs up. Non-blocking — never raises."""
    if not settings.ADMIN_EMAIL or not settings.SMTP_USER:
        return False
