...
class Thought(Base):
    ...
    author     = Column(String(200), default="William")
...
def list_thoughts(category: Optional[str] = None, db: Session = Depends(get_db)):
...
def create_thought(body: ThoughtIn, db: Session = Depends(get_db), admin=Depends(get_admin)):
    now = datetime.utcnow()
    t = Thought(
...
def update_thought(thought_id: int, body: ThoughtUpdate, db: Session = Depends(get_db), admin=Depends(get_admin)):
...
def delete_thought(thought_id: int, db: Session = Depends(get_db), admin=Depends(get_admin)):
    t = db.query(Thought).filter(Thought.id == thought_id).first()
    if not t:
        raise HTTPException(404, "Thought not found.")
