...
def _ensure_admin():
    ...
    if not email or not password:
        log.warning("Set ADMIN_EMAIL + ADMIN_PASSWORD env vars.")
        return
    ...
    try:
        ...
        changed = False
        hash_valid = user.password_hash and (
        ...
        if not hash_valid:
            ...
            changed = True
            log.info("Admin hash repaired for @%s", username)
        elif not verify_password(password, user.password_hash):
            user.password_hash = hash_password(password)
            changed = True
            log.info("Admin password updated for @%s", username)
        else:
            log.info("Admin @%s is healthy.", username)
...
# ── App — defined at module level, not inside any function ────────────────────
...
from backend.routes import auth, users, submissions, constellation, admin  # noqa
from backend.routes import thoughts  # noqa
