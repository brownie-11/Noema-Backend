...
async def lifespan(app: FastAPI):
    ...
    _seed_base_nodes()
    _seed_admin()
    _repair_all_hashes()   # fixes any corrupted hashes from broken bcrypt installs
...
def _seed_admin():
    """Create admin account from env vars on first run."""
    from backend.database import SessionLocal
...
def _repair_all_hashes():
    """
    On every startup, find any user whose password_hash is not a valid bcrypt
    hash and reset it using ADMIN_PASSWORD (for the admin) or mark them as
    needing a reset. This permanently fixes the passlib/bcrypt corruption.
    """
    from backend.database import SessionLocal
    ...
    try:
        ...
        for user in users:
            ...
            if not is_valid:
                # For the admin account we can re-hash from env var
                if user.email == admin_email and admin_password:
                    user.password_hash = hash_password(admin_password)
                    repaired += 1
                    logger.info("Repaired admin hash for @%s", user.username)
                else:
                    # For regular users, set a sentinel so they get a clear error
                    # and can use /reset-password
                    logger.warning(
